...
async def build_convert_tab(
    *,
    ...
    legacy_source = cast(str, await pref_get("conv.last_source", ""))
    legacy_path = Path(legacy_source) if legacy_source else None
    default_file = (
        legacy_source
        if legacy_path is not None and legacy_path.suffix.lower() == ".pdf"
        else ""
    )
    default_folder = (
        legacy_source if legacy_path is not None and not default_file else ""
    )

    async def conv_files_changed(files: list[Path]) -> None:
        await pref_set("conv.last_file", str(files[0]) if files else "")

    conv_drop_field = PdfDropField(
        file_picker=file_picker,
        allow_multiple=False,
        on_files_changed=conv_files_changed,
        log=log,
        select_button_text="選択",
    )
    last_file = cast(
        str,
        await pref_get("conv.last_file", default_file),
    )
    if last_file:
        await conv_drop_field.add_paths(
            [last_file],
            source="保存済み設定",
            notify=False,
        )

    conv_folder_input = ft.TextField(
        label="入力フォルダ",
        hint_text="フォルダを選択してください",
        read_only=True,
        expand=True,
    )
    conv_folder_input.value = cast(
        str,
        await pref_get("conv.last_folder", default_folder),
    )
    conv_folder_pick_btn = ft.Button(content="選択", width=90)
    ...
    async def conv_mode_changed(e: ft.Event) -> None:
        ...
        file_mode = conv_mode.value == "file"
        conv_drop_field.visible = file_mode
        conv_folder_input.visible = not file_mode
        conv_folder_pick_btn.visible = not file_mode
        conv_drop_field.update()
        conv_folder_input.update()
        conv_folder_pick_btn.update()
    ...
    async def pick_conv_folder(e: ft.Event) -> None:
        res = await choose_dir_async()
        if isinstance(res, str) and res:
            conv_folder_input.value = res
            await pref_set("conv.last_folder", res)
            conv_folder_input.update()

    async def do_convert(e: ft.Event) -> None:
        source = (
            conv_drop_field.value
            if conv_mode.value == "file"
            else (conv_folder_input.value or "").strip()
        )
        if not source:
        ...
        conv_drop_field.disabled = True
        conv_folder_pick_btn.disabled = True
        cancel_event = Event()
        try:
        ...
        try:
            ...
            conv_drop_field.disabled = False
            conv_folder_pick_btn.disabled = False
            conv_run_btn.update()
            conv_drop_field.update()
            conv_folder_pick_btn.update()
    ...
    conv_folder_input.visible = not file_mode
    conv_folder_pick_btn.visible = not file_mode
    conv_folder_pick_btn.on_click = pick_conv_folder
    ...
    tab_convert = ft.Container(
        ...
        content=ft.ListView(
            ...
            controls=[
                ...
                conv_drop_field,
                ft.Row([conv_folder_input, conv_folder_pick_btn], spacing=8),
                ft.Row(
                    [
...
async def build_split_tab(
    *,
    ...
    async def split_files_changed(files: list[Path]) -> None:
        await pref_set("split.last_source", str(files[0]) if files else "")

    split_drop_field = PdfDropField(
        file_picker=file_picker,
        allow_multiple=False,
        on_files_changed=split_files_changed,
        log=log,
        select_button_text="選択",
    )
    last_source = cast(str, await pref_get("split.last_source", ""))
    if last_source:
        await split_drop_field.add_paths(
            [last_source],
            source="保存済み設定",
            notify=False,
        )
    ...
    async def do_split(e: ft.Event) -> None:
        ...
        split_drop_field.disabled = True
        cancel_event = Event()
        try:
        ...
        try:
            ...
            split_drop_field.disabled = False
            split_run_btn.update()
            split_drop_field.update()
...
async def build_merge_tab(
    *,
    ...
    merge_folder_row = ft.Row([merge_folder, btn_pick_folder], spacing=8)

    def merge_files_changed(files: list[Path]) -> None:
        return None

    merge_drop_field = PdfDropField(
        file_picker=file_picker,
        allow_multiple=True,
        on_files_changed=merge_files_changed,
        log=log,
        select_button_text="PDFを追加",
    )
    ...
    async def merge_mode_changed(e: ft.Event) -> None:
        ...
        files_mode = merge_mode.value == "files"
        merge_drop_field.visible = files_mode
        merge_folder_row.visible = not files_mode
        merge_drop_field.update()
        merge_folder_row.update()
    ...
    async def do_merge(e: ft.Event) -> None:
        ...
        merge_drop_field.disabled = True
        cancel_event = Event()
        try:
        ...
        try:
            ...
            result = await pdf_marge(
                merge_mode.value,
                merge_drop_field.paths,
                merge_folder.value if merge_folder else "",
                out_path,
                cancel_event,
            )
            ...
            merge_drop_field.disabled = False
